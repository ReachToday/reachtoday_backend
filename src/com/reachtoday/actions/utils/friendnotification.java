	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			JSONArray arr=new JSONArray();
				...
				resultSet = statement.executeQuery("select user_friends_login_users_mail_id,user_friends_isactive from user_friends where user_friends_email ='"+request.getParameter("currentloggedmail").trim().toLowerCase()+"'");
			...
			while(resultSet.next())
			{
				...
				if(resultSet.getString("user_friends_isactive")!=null){
					...
					statement = connect.createStatement();
					System.out.println("select login_users_username from login_users where  login_users_mail_id ="+resultSet.getString("user_friends_login_users_mail_id").trim().toLowerCase());
					subresultSet = statement.executeQuery("select login_users_username from login_users where  login_users_mail_id = '"+resultSet.getString("user_friends_login_users_mail_id").trim().toLowerCase()+"'");
